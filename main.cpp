...
TEST(bimap, custom_comparator) {
  bmp::bimap<int, int, std::greater<>> b;
  b.insert(3, 4);
  b.insert(1, 5);
  b.insert(10, -10);
...
TEST(bimap, custom_parametrized_comparator) {
  ...
  bmp::bimap<vec, vec, vector_compare, vector_compare> b(
...
template class bmp::bimap<int, non_default_constructible>;
template class bmp::bimap<non_default_constructible, int>;
...
TEST(bimap_randomized, comparison) {
  ...
  bmp::bimap<uint32_t, uint32_t> b1;
  bmp::bimap<uint32_t, uint32_t> b2;
